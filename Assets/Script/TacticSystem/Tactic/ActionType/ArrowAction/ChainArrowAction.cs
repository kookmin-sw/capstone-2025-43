...
[CreateAssetMenu(menuName = "TacticSystem/Action/ChainArrow")]
public class ChainArrowAction : ActionType
{
    ...
    public override void Execute(Character user, List<Character> targets)
    {
        ...
        {
            user.StartTrackedCoroutine(MoveAndAction(user, target, user.agent, AttackWithChainArrow));
        }
    }

    private void AttackWithChainArrow(Character user, Character target)
    {
